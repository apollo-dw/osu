...
namespace osu.Game.Rulesets.Osu.Difficulty.Skills
{
    ...
    public class Aim : OsuStrainSkill
    {
        ...
        private double strainValueOf(DifficultyHitObject current)
        {
            ...
            double prevVelocity = osuLastObj.Distance / osuLastObj.StrainTime;
                ...
                if (osuCurrObj.Angle != null && osuLastObj.Angle != null && osuLastLastObj.Angle != null)
                {
                    ...
                    else
                    {
                        acuteAngleBonus *= calcAcuteAngleBonus(lastAngle) // Multiply by previous angle, we don't want to buff unless this is a wiggle type pattern.
                                           ...
                                           * Math.Pow(Math.Sin(Math.PI / 2 * (Math.Clamp(osuCurrObj.Distance, 50, 100) - 50) / 50), 2); // Buff distance exceeding 50 (radius) up to 100 (diameter).
                    }
            ...
            if (Math.Max(prevVelocity, currVelocity) != 0)
            {
                ...
                double nonOverlapVelocityBuff = Math.Abs(prevVelocity - currVelocity)
                                                ...
                                                * Math.Pow(Math.Sin(Math.PI / 2 * Math.Min(1, Math.Min(osuCurrObj.Distance, osuLastObj.Distance) / 100)), 2);
            ...
            if (osuCurrObj.Clickable && osuLastObj.Clickable)
            {
