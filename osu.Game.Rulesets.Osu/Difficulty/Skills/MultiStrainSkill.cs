...
namespace osu.Game.Rulesets.Osu.Difficulty.Skills
{
    public abstract class MultiStrainSkill : Skill
    {
        ...
        /// <summary>
        /// The final multiplier to be applied to the skill after all other calculations.
        /// </summary>
        ...
        protected MultiStrainSkill(Mod[] mods)
            : base(mods)
        {
            strainPeaks = new List<double>[ConcurrentStrainCount];

            for (int i = 0; i < ConcurrentStrainCount; i++)
                strainPeaks[i] = new List<double>();

            currentSectionPeak = new double[ConcurrentStrainCount];
        }

        /// <summary>
        /// Returns the strain values at <see cref="DifficultyHitObject"/>. This value is calculated with or without respect to previous objects.
        /// </summary>
        protected abstract double[] StrainValuesAt(DifficultyHitObject current);
        ...
        public sealed override void Process(DifficultyHitObject current)
        {
            ...
            while (current.StartTime > currentSectionEnd)
            {
                ...
                {
                    saveCurrentPeak(i);
                    startNewSectionFrom(currentSectionEnd, current, i);
                }

                currentSectionEnd += SectionLength;
            }

            double[] strains = StrainValuesAt(current);

            for (int i = 0; i < ConcurrentStrainCount; i++)
                currentSectionPeak[i] = Math.Max(strains[i], currentSectionPeak[i]);
        }
        ...
        {
            Console.WriteLine(currentSectionPeak[offset]);
            strainPeaks[offset].Add(currentSectionPeak[offset]);
        }
        ...
        private void startNewSectionFrom(double time, DifficultyHitObject current, int offset)
        {
            ...
            currentSectionPeak[offset] = CalculateInitialStrain(time, current, offset);
        }
        ...
        /// </summary>
        public IEnumerable<double> GetCurrentStrainPeaks(int offset) => strainPeaks[offset].Append(currentSectionPeak[offset]);
        ...
        public double DifficultyValue(int offset)
        {
            double difficulty = 0;
            double weight = 1;
            ...
            var peaks = GetCurrentStrainPeaks(offset).Where(p => p > 0);
